...
class TuckerRegressor():
    ...
    def __init__(self, weight_ranks, tol=10e-7, reg_W=1, n_iter_max=100, random_state=None, verbose=1):
        self.weight_ranks = weight_ranks # shape of the kernel
        self.tol = tol
        self.reg_W = reg_W
    ...
    def fit2(self, X, y, reg_W_vec=None):
        """Fits the model to the data (X, y)
        ...
        self
        """
        if reg_W_vec is None:
            reg_W_vec = T.ones(X.ndim - 1)
        else:
            assert len(reg_W_vec) == X.ndim - 1, "The length of `reg_W_vec` is wrong"
            reg_W_vec = T.tensor(reg_W_vec)
                ...
                phi = partial_tensor_to_vec(
                            T.dot(partial_unfold(X, i),
                                  T.dot(kronecker(W, skip_matrix=i),
                                          T.transpose(unfold(G, i)))))
                # Regress phi on y: we could call a package here, e.g. scikit-learn
                # regularization: drop the ith element and transformed into matrix
                # reg_W_mat = T.diag([T.prod(reg_W_vec) / reg_W_vec[i]] * phi.shape[1])
                reg_W = T.prod(reg_W_vec) / reg_W_vec[i] # * T.tensor(np.eye(phi.shape[1]))
                inv_term = T.dot(T.transpose(phi), phi) +\
                     reg_W * T.tensor(np.eye(phi.shape[1]), **T.context(X)) ##revised
                W_i = vec_to_tensor(T.solve(inv_term, T.dot(T.transpose(phi), y)),
                                    (X.shape[i + 1], G.shape[i]))
                W[i] = W_i

            # reg_W_mat = T.diag([T.prod(reg_W_vec)] * phi.shape[1])
            phi = T.dot(partial_tensor_to_vec(X), kronecker(W))
            reg_W = T.prod(reg_W_vec) # * T.tensor(np.eye(phi.shape[1]))
            G = vec_to_tensor(T.solve(T.dot(T.transpose(phi), phi) +\
                                        reg_W * T.tensor(np.eye(phi.shape[1]), **T.context(X)), ##revised
                                      T.dot(T.transpose(phi), y)), G.shape)
        ...
        self.weight_tensor_ = weight_tensor_
        self.tucker_weight_ = (G, W)
        self.vec_W_ = tucker_to_vec((G, W)) # G * np.prod(np.ix_(a,b,c))
        self.n_iterations_ = iteration + 1
        self.norm_W_ = norm_W
    ...
    def fit_new(self, X, y, model=Ridge(fit_intercept=False)):
        """Fits the model to the data (X, y)
        ...
        y : array of shape (n_samples)
            labels associated with each sample
        model: sklearn model to regress phi on y 
        ...
        for iteration in range(self.n_iter_max):
            ...
            for i in range(len(W)):
                ...
                #                     (X.shape[i + 1], G.shape[i]))
                model.fit(phi, y)
                W_i = vec_to_tensor(T.tensor(model.coef_), (X.shape[i + 1], G.shape[i]))
                W[i] = W_i
            ...
            #                           T.dot(T.transpose(phi), y)), G.shape)
            model.fit(phi, y)
            G = vec_to_tensor(T.tensor(model.coef_), G.shape)
