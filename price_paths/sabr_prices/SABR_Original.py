...
def sabrMC(F0=0.04, sigma0=0.07, alpha=0.5, beta=0.25, rho=0.4, psi_threshold=2., n_years=1.0, T=252, N=1000,
           trapezoidal_integrated_variance=False, s=0.0):
    ...
    for n in range(0, N):
        for ti in range(1, T):
            ...
            if Ft[ti - 1, n] == 0.:
                Ft[ti, n] = 0.
                continue
            a = (1. / v_t[ti - 1, n]) * (((Ft[ti - 1, n] ** (1. - beta)) / (1. - beta) + (rho / alpha) * (
            ...
            if m >= 0 and psi <= psi_threshold:
                ...
                d = m / (1. + e2)
                Ft[ti, n] = np.power(((1. - beta) ** 2) * v_t[ti - 1, n] * d * ((math.sqrt(e2) + Z[ti - 1, n]) ** 2),
                                     1. / (2. * (1. - beta)))
...
if __name__ == '__main__':
    Ft = sabrMC(N=1000, F0=0.1, sigma0=0.1)
    pyplot.plot(Ft)
    pyplot.show()